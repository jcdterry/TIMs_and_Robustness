...
# Maths and Analyses:
library(deSolve);library(igraph)
library(numDeriv);library(rootSolve)
library(NetIndices)

# Graphics:
library(pbapply);library(knitr)
require(cowplot);library(mvbutils);
library(xtable)

# Utilities:
library(Rcpp);library(microbenchmark);
library(pracma);library(parallel);library(R.utils)
library(tidyverse)
```
...
ParallelRun <- function(DATA,FUNC,Name, ...){
  c1<- makeCluster(4)
  clusterEvalQ(c1,{require(dplyr);require(rootSolve);require(deSolve);require(igraph);require(purrr)})->tmp
...
### Run the models:
```{r, eval=FALSE}
load('Output from Simulations/TIMModelsInflex')
...
load('Output from Simulations/NTEModelsInflex')
